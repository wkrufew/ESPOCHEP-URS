...
class CertificadoEquipo extends Component
{
    public $inicio;
    public $fin;
    public $search1 = '';
    ...
    public function cambiarAsignacion()
    {
        ...
        if (!$certificadosExistentes) {
            $this->setFlashMessage('error', 'Los certificados en el rango especificado no existen.');
            return;
        }
        ...
        if ($certificadosAsignados) {
            $this->setFlashMessage('error', 'Algunos certificados en el rango ya están asignados al nuevo equipo.');
            return;
        }
        ...
        Certificado::whereBetween('code', [$inicio, $fin])
            ->where('equipment_id', $equipoActual->id)
            ->update(['equipment_id' => $nuevoEquipo->id]);

        $this->setFlashMessage('success', 'Asignación de certificados actualizada exitosamente.');
    ...
    public function render()
    {
        ...
        return view('livewire.admin.certificado-equipo', ['equipments1' => $equipments1, 'equipments2' => $equipments2]);
    }
}
